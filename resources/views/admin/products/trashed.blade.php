...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  __('messages.Products') => route('admin.products.index'),
  __('messages.Trashed Products') => 'active',
  ],
  ])
              ...
              <div class="d-flex justify-content-between mt-2">
                ...
                <div class="btn-group" role="group" aria-label="Basic example">
                  ...
                  <button type="button" class="btn btn-outline-primary waves-effect" data-bs-toggle="modal"
                    ...
                    {{ __('messages.Restore All') }}
                    <i class="fa-solid fa-trash-restore"></i>
                  </button>

                  <div class="modal fade" id="restoreAllModal" tabindex="-1" aria-labelledby="restoreAllModalLabel"
                    aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="restoreAllModalLabel">{{ __('messages.Confirm Action') }}</h5>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                          {{ __('messages.Are you sure you want to restore all products?') }}
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{
                            __('messages.Cancel') }}</button>
                          <form action="{{ route('admin.products.restore_all') }}" method="POST">
                            @csrf
                            @method('POST')
                      ...
                      <td class="dynamic-column" data-column="actions">
                        <div class="dropdown">
                          ...
                          <div class="dropdown-menu">
                            ...
                            @if ($item->deleted_at)
                            @if (auth()->user()->type == \App\Enums\UserRoleEnum::Admin)
                            <a data-url="{{ route('admin.products.restore', $item->id) }}"
                              data-text_btn_confirm="{{ __('messages.Confirm') }}"
                              data-text_btn_cancel="{{ __('messages.Cancel') }}" data-method="post"
                              data-message="{{ __('messages.Are you sure you want to restore?') }}"
                              class="dropdown-item btn-action" href="javascript:void(0);"><i
                                class="fa-solid fa-trash-restore"></i>
