  ...
  <div class="row justify-content-center">
    ...
    <div class="col-md-12">
      ...
      <div class="card mb-4">
        ...
        <form class="card-body" action="{{ route('admin.products.update', $item->id) }}" method="POST"
          ...
          <div class="row g-3">
            <div class="nav-align-top  mb-4">
              <ul class="nav nav-tabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                  <button type="button" class="nav-link {{ $loop->index == 0 ? 'active' : '' }}" role="tab"
                    data-bs-toggle="tab" data-bs-target="#navs-{{ $lang->id }}" aria-controls="navs-{{ $lang->id }}"
                    aria-selected="true">
                    {{ $lang->native_name }}
                  </button>
                </li>
              ...
              <div class="tab-content p-0">
                @foreach ($languages as $lang)
                <div class="tab-pane fade {{ $loop->index == 0 ? 'active show' : '' }}  " id="navs-{{ $lang->id }}"
                  role="tabpanel">
                  <div class="row">
                    <div class="col-md-12 mt-3">
                      ...
                      <input type="text" id="name_{{ $lang->code }}" data-slug_name="slug_{{ $lang->code }}"
                        ...
                        value="{{ isset($translations[$loop->index]) ? $translations[$loop->index]->name : '' }}" />
                    ...
                    <div class="col-md-12 mt-3">
                      <label class="form-label mb-1"
                        for="description_{{ $lang->code }}">{{__('messages_303.description') }}({{ $lang->code }}
                        )</label>
                      <textarea id="description_{{ $lang->code }}"
                        ...
                        name="description_{{ $lang->code }}">{{ isset($translations[$loop->index]) ? $translations[$loop->index]->description : '' }}</textarea>
                ...
                <div class="row g-3">
                  <div class="col-md-6 p-2">
                    ...
                    <select name="category_id"
                      ...
                      id="select_category_id" data-icon-base="ti" data-tick-icon="ti-check" data-style="btn-default"
                      data-live-search="true">
                      <option selected disabled>{{ __('messages.Brand') }}</option>
                      @foreach ($categories as $category)
                      <option value="{{ $category->id }}" @selected(old('category_id', $item->category_id) ==
                        $category->id)>
                        {{ $category->name }}
                      </option>
                      @endforeach
                  ...
                  <div class="col-md-6 p-2">
                    ...
                    <select name="sub_category_id"
                      ...
                      id="select_sub_category_id" data-icon-base="ti" data-tick-icon="ti-check" data-style="btn-default"
                      data-live-search="true">
                      <option selected disabled>{{ __('messages.category name') }}</option>
                      @foreach ($sub_categories as $sub_category)
                      <option value="{{ $sub_category->id }}" @selected(old('sub_category_id', $item->sub_category_id)
                        == $sub_category->id)>
                        {{ $sub_category->name }}
                      </option>
                      @endforeach
                  ...
                  <div class="col-md-6 p-2">
                    ...
                    <select name="supplier_id"
                      ...
                      id="select_supplier_id" data-icon-base="ti" data-tick-icon="ti-check" data-style="btn-default"
                      data-live-search="true">
                      <option selected disabled>{{ __('messages_301.Supplier name') }}</option>
                      @foreach ($suppliers as $supplier)
                      <option value="{{ $supplier->id }}" @selected(old('supplier_id', $item->supplier_id) ==
                        $supplier->id)>
                        {{ $supplier->name }}
                      </option>
                      @endforeach
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input value="{{ old('barcode', $item->barcode) }}"
                      class="form-control @error('barcode') is-invalid @enderror" type="text" name="barcode">
                    @error('barcode')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input class="form-control @error('code')
                                            is-invalid
                                          @enderror" type="text" name="code" value="{{ old('code', $item->code) }}">
                    @error('code')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input class="form-control @error('qty') is-invalid @enderror" type="number" name="qty"
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input class="form-control @error('price') is-invalid @enderror" type="text" id="formFile"
                      name="tax" value="{{ old('tax', $item->tax) }}">
                    @error('price')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  ...
                  <div class="col-md-6 mb-3">
                    <label for="formFile" class="form-label">{{ __('messages.Price Before Tax') }}</label>
                    <input class="form-control @error('price_before_tax') is-invalid @enderror" type="text"
                      id="formFile" name="price_before_tax"
                      value="{{ old('price_before_tax', $item->price_before_tax) }}">
                    @error('price_before_tax')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input class="form-control @error('price') is-invalid @enderror" type="text" id="formFile"
                      name="price" value="{{ old('price', $item->price) }}" readonly>
                    @error('price')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
