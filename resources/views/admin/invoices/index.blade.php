                    ...
                    <div class="col-12">
                        <div class=" mb-4">
                            <div class="card-body">
                                <div class="d-flex">
                                    @if ($doctor_sel)
                                        <span class="badge bg-label-success mx-2"><i data-input_name="select_doctor_id"
                                                class="ti ti-x cursor-pointer btn_remove_filter"></i>{{ __('messages.Doctor') }}
                                            : {{ $doctor_sel }}</span>
                                    @endif
                                    ...
                                    {{--
                                    @if ($hall_name)
                                        <span class="badge bg-label-success mx-2"><i data-input_name="select_hall_id" class="ti ti-x cursor-pointer btn_remove_filter"></i>{{ __("messages.Hall") }} : {{ $hall_name }}</span>
                                    @endif

                                    @if ($show_name)
                                        <span class="badge bg-label-success mx-2"><i data-input_name="select_show_id" class="ti ti-x cursor-pointer btn_remove_filter"></i>{{ __("messages.Show") }} : {{ $show_name }}</span>
                                    @endif

                                    @if ($date)
                                        <span class="badge bg-label-success mx-2"><i data-input_name="select_show_date_id" class="ti ti-x cursor-pointer btn_remove_filter"></i>{{ __("messages.Show Date") }} : {{ \Carbon\Carbon::parse($date)->format('Y-m-d h:i:s A') }}</span>
                                    @endif --}}
                                ...
                                <div class="p-3 d-flex justify-content-between">
                                    ...
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="{{ route('admin.invoices.export') }}" class="btn btn-outline-primary waves-effect text-primary"><span class="d-none d-sm-block">{{ __("messages.EXPORT XLS") }}</span><i class="fa-solid fa-file-export"></i></a>
                                        @can('create_invoice')
                                        <a href="{{ route('admin.invoices.create') }}" class="btn btn-outline-primary waves-effect text-primary"><span class="d-none d-sm-block">{{ __("messages.ADD NEW ") }}</span><i class="fa-solid fa-circle-plus "></i></a>
                                        @endcan
                                    </div>
                                    ...
                                    <form id="form_filter">
                                        <div class="row">
                                            <div class=" col-4 mt-4">
                                                <label
                                                    class="mb-1">{{ __('messages.Customer name, phone number') }}</label>
                                            ...
                                            <div class=" col-4 mt-4">
                                                ...
                                                <select @disabled(auth()->user()->type == App\Enums\UserRoleEnum::Doctor) name="doctor_id" class="form-control select2 select_doctor_id">
                                                    <option value="">--------</option>
                                                    @if (auth()->user()->type == App\Enums\UserRoleEnum::Doctor)
                                                    <option value="" selected>{{ auth()->user()->name }}</option>
                                                    @else
                                                    @foreach ($doctors as $item_doctor)
                                                        <option @selected(request('doctor_id') == $item_doctor->id)
                                                            value="{{ $item_doctor->id }}">{{ $item_doctor->name }} -
                                                            {{ $item_doctor->email }}</option>
                                                    @endforeach
                                                    @endif
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td style="min-width: 200px">
                                    @if (auth()->user()->type == App\Enums\UserRoleEnum::Doctor)
                                    <span class="badge bg-label-{{ App\Http\Helpers\HelperApp::get_color_status($item->status) }} me-1">{{ __('messages.' . $item->status) }}</span>
                                    @else
                                        @can('review_invoice')
                                        <div class="col-12">
                                            @include('admin.invoices.status_selected' , ['invoice_item_status'=>$item->status , 'invoice_item_id'=>$item->id])
                                        </div>
                                        @else
                                        <span class="badge bg-label-{{ App\Http\Helpers\HelperApp::get_color_status($item->status) }} me-1">{{ __('messages.' . $item->status) }}</span>
                                        @endcan
                                ...
                                <td style="min-width: 200px">
                                    @if (auth()->user()->type == App\Enums\UserRoleEnum::Doctor)
                                    <span class="badge bg-label-{{ App\Http\Helpers\HelperApp::get_color_status($item->status) }} me-1">{{ $item->payment_type  ?  $item->payment_type  : 'Unpaid' }}</span>
                                    @else
                                        @can('review_invoice')
                                        <div class="col-12">
                                            @include('admin.invoices.payment_selected' , ['invoice_item_payment_type'=>$item->payment_type , 'invoice_item_id'=>$item->id])
                                        </div>
                                        @else
                                        <span class="badge bg-label-{{ App\Http\Helpers\HelperApp::get_color_status($item->status) }} me-1">{{ __('messages.' . $item->status) }}</span>
                                        @endcan
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            href="{{ route('admin.invoices.pdf', $item->id) }}"><i
                                            ...
                                            @can('edit_invoice')
                                                <a class="dropdown-item"
                                                    href="{{ route('admin.invoices.edit', $item->id) }}"><i
                                            ...
                                            @if (auth()->user()->type->value == 'admin')
                                                @can('review_invoice')
                                                    @if ($item->reviewer && $item->status == \App\Enums\OrderStatusEnum::Draft->value)
                                                        <a data-url="{{ route('admin.invoices.review', $item->id) }}"
                                                            href="javascript:void(0);"
                                                            data-text_btn_confirm="{{ __('messages.Confirm') }}"
                                                            data-text_btn_cancel="{{ __('messages.Cancel') }}"
                                                            data-method="post"
                                                            data-message="{{ __('messages_301.Reviewed') }}"
                                                            class="dropdown-item btn-action"><i class="ti ti-eye-check"></i>
