        ...
        <div class="card">
                            ...
                            <div class="col-md-3 col-12">
                                <div class="card mb-4">
                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item text-center h5">{{ __('messages.Doctors commission') }}
                                        </li>
                                        <li class="list-group-item text-center">
                                            {{ $data->sum('doctor_commission_value') }} SAR
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            ...
                            <div class=" col-3 mt-4">
                                ...
                                <select name="year" class="form-control select2 select_year">
                                    ...
                                    @foreach ($years as $year)
                                        <option @if ($search_year == $year) selected @endif
                                            value="{{ $year }}">
                                            {{ $year }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class=" col-3 mt-4">
                                ...
                                <select name="month" class="form-control select2 select_month">
                                    @foreach ($months as $month)
                                        <option @if ($search_month == $month) selected @endif
                                            value="{{ $month }}">
                                            {{ $month }}</option>
                                    @endforeach
                                </select>
            ...
            <div class="d-flex justify-content-end p-3">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{ route('admin.doctors.export' , $doctor->id) }}?{{ request()->getQueryString() }}" class="btn btn-outline-primary waves-effect text-primary"><span class="d-none d-sm-block">{{ __("messages.EXPORT XLS") }}</span><i class="fa-solid fa-file-export"></i></a>
                </div>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('messages_301.Commission') }}</th>
                        </tr>
                    </thead>
