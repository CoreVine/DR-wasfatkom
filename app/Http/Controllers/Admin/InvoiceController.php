...
وصفتك إلى باب منزلك
  ...
  public function create()
  {
    ...
    return view("admin.invoices.create", compact('products', 'doctors', 'reviewers', 'formulations', 'favorites_ids_str', 'favorites', 'packages'));
  }
  ...
  public function store(InvoiceRequest $request)
  {
    ...
    $discount = $item_data['invoice_total_discount'] ?? 0;
    ...
    if ($request->packages && count($request->packages)) {
      ...
      $discount += $package_data['invoice_total_discount'] ?? 0;

      \Log::info('Package Data:', $package_data);
    }

    if ($request->formulations && count($request->formulations)) {
      $formulation_data = $this->get_formulation_data($request, $invoice->id);
      InvoiceFormulation::insert($formulation_data['data']);

      $sub_total += $formulation_data['invoice_total'];
      $total += $formulation_data['invoice_total'];
      $discount += $formulation_data['invoice_total_discount'] ?? 0;

      \Log::info('Formulation Data:', $formulation_data);
    }
    ...
    \Log::info('Final Invoice Total:', ['total' => $total]);
  ...
  public function edit(string $id)
  {
    ...
    $user_id = auth()->id();
    $user = User::findOrFail($user_id);
    $item = Invoice::with(['doctor', 'invoice_items', 'invoice_formulations'])->findOrFail($id);
    $packages = Package::select("id", "price")->get();
    $formulations = Formulation::select("id", "price")->get();
    ...
    return view('admin.invoices.edit', compact('item', 'formulations', 'doctors', 'products', 'reviewers', 'favorites_ids_str', 'favorites', 'packages'));
  }
  ...
  public function update(InvoiceRequest $request, string $id)
  {
    ...
    try {
      ...
      $invoice = Invoice::with(['doctor', 'invoice_items', 'invoice_formulations'])->findOrFail($id);
      ...
      $data = [
        'doctor_id' => $request->doctor_id,
        'review_id' => auth()->user()->type->value == UserRoleEnum::Admin->value ? $request->review_id : $invoice->review_id,
        'client_name' => $request->client_name,
        'client_mobile' => $request->client_mobile,
        'client_location' => $request->client_location,
        'coupon_id' => $request->coupon_id ?: null,
        'notes' => $request->notes,
        'doctor_commission' => auth()->user()->type->value == UserRoleEnum::Admin->value ? $request->doctor_commission : $invoice->doctor_commission,
      ];
      ...
      \Log::info('Final Invoice Update:', $data);
      $invoice->update($data);
  ...
  private function get_formulation_data($request, $invoice)
  {
    ...
    foreach ($request->formulations as $item) {
      if (isset($item['formulation_id']) && $item['formulation_id']) {
        $formulation = Formulation::where('id', $item['formulation_id'])->select('price')->first();
        if (!$formulation) abort(404);
        $total = $formulation->price * ($item['qty'] ?? 1);
        $total_before_discount = $formulation->price * ($item['qty'] ?? 1);
        $data[] = [
          'invoice_id' => $invoice,
